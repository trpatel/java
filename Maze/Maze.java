...
public class Maze {
	...
	c = new DisjointSets(horiz * vert); //creates a new DisjointSets with the dimensions
	Wall[] allwalls = new Wall[(horiz - 1) * vert + (vert - 1) * horiz]; //creates an array of the walls
	int z = 0;
	for(int q = 0; q < (horiz - 1); q++){
	    for(int r = 0; r < vert; r++){
		allwalls[z] = new Wall(q, r, Wall.Ver);
		z++;
	    }
	}
	for(int q = 0; q < horiz; q++){
	    for(int r = 0; r < (vert - 1); r++){
		allwalls[z] = new Wall(q, r, Wall.Hor);
		z++;
	    }
	}
	//the order of the walls are random
	for(int t = allwalls.length; t > 1; t--){
	    int target = randInt(t);
	    Wall targetexchange = allwalls[target];
	    allwalls[target] = allwalls[t - 1];
	    allwalls[t - 1] = targetexchange;
	}
	for(z = 0; z < allwalls.length; z++){ //eliminate unnecessary walls
	    Wall cwall = allwalls[z];
	    int x1;
	    int y1;
	    int x2;
	    int y2;
	    ...
	    int path1 = c.find(targetindex(x1, y1));
	    int path2 = c.find(targetindex(x2, y2));
	    if(path1 != path2){
		if(cwall.type == Wall.Hor){
		    hWalls[cwall.x][cwall.y]= false;
		}else{
		    vWalls[cwall.x][cwall.y] = false;
		}
		c.union(path1, path2);
	    }
	}
